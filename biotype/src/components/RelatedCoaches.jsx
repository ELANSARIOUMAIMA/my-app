...
const RelatedCoaches = ({type,coId}) => {
    const {coaches} = useContext(AppContext)
    const navigate = useNavigate()

    const[relCo,setRelCo]=useState([])

    useEffect(()=>{
        if(coaches.length > 0 && type){
            const coachesData = coaches.filter((co)=>co.type.includes(type) && co._id !== coId )
            setRelCo(coachesData)
        }
  ...
  return (
    <div className='flex flex-col items-center my-16 gap-4 text-gray-800  md:mx-10'>
        <h1 className=" text-3xl font-medium">Related Coaches  </h1>
        <p className="sm:w-1/2 text-center text-sm">Simply browse through our extensive list of trusted doctors. </p>
        <div className=" w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0">
            {relCo.slice(0,4).map((item,index)=>(
                <div  onClick={()=>{navigate(`/appointments/${item._id}`);scrollTo(0,0)}} key={index} className=" border-2 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500 ">
